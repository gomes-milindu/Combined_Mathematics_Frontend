...
export default function StudentTable({ toggleMenu, onStudentsLoaded }) {
  ...
  const fetchStudents = async (p) => {
    ...
    try {
      ...
      // Notify parent about loaded students for action menu
            ...
            {loading ? (
              ...
              students.map((items, index) => (
                ...
                >
                  ...
                  <td className="px-6 md:px-2 lg:px-6 py-4">
                    <span
                      className={`inline-flex items-center rounded-full px-3 py-1 md:px-2 md:py-0.5 lg:px-3 lg:py-1 text-xs font-medium ${items.isActive ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}
                    >
                  ...
                  <td className="px-6 md:px-2 lg:px-6 py-4 text-right relative">
                    <button
                      onClick={(e) => toggleMenu && toggleMenu(index, e, items)}
